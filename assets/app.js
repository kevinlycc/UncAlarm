...
document.addEventListener('DOMContentLoaded', () => {
    ...
    dismissButton.addEventListener('click', () => {
        socket.emit('dismiss_alert', {}); // Notify backend to dismiss
        hideAlert();
    });
...
function initSocketIO() {
    ...
    socket.on('fall_alert', (state) => {
        handleFallAlert(state);
    });

    socket.on('alert_status', (statusText) => {
        updateAlertStatus(statusText);
    });

}

function handleFallAlert(state) {
    switch(state) {
        case 'LOCAL_WARNING_STARTED':
            showAlert("LOCAL FALL WARNING", "We detected a possible fall. Please dismiss if you are okay.");
            break;
        case 'FAMILY_NOTIFIED':
            showAlert("FAMILY NOTIFIED", "Timed out. Family has been contacted. Help is on the way.", true);
            alertTitle.style.color = "#FF9800"; // Alert color change
            break;
        case 'EMERGENCY_SERVICES_CALLED':
            showAlert("EMERGENCY CALLED", "Emergency services have been contacted.", true);
            alertTitle.style.color = "#FF0000"; 
            break;
...
function showAlert(title, description, isCritical = false) {
    alertOverlay.style.display = 'flex';
    alertOverlay.classList.toggle('active', true);
    alertTitle.innerText = title;
    alertDescription.innerText = description;
    
    // Add critical styling if family/emergency notified
    document.body.classList.toggle('alert-notified', isCritical);
}

function hideAlert() {
    alertOverlay.style.display = 'none';
    alertOverlay.classList.remove('active');
    document.body.classList.remove('alert-notified');
}

function updateAlertStatus(statusText) {
    // Extract numbers from status text for the big timer if available
    const timeMatch = statusText.match(/\d+s/);
    if (timeMatch) {
        alertTimer.innerText = timeMatch[0].replace('s', '');
    }
    
    // Update description to show the latest status detail (e.g., activity detected bonus)
    if (statusText.includes("Activity detected")) {
        alertDescription.innerText = statusText;
    }
}
